module.exports = [
	...
	'[hit]После обмена серией стремительных ударов с(о) {victim}, {actor} наконец наш(ел|ёл|ла|ло|ли) брешь в защите и нанес(ла|ло|ли) {value} единиц(у|ы) урона.',
	...
	'[hit]{actor} удачно париру(е|ю)т удар {victim}, и он(а|о|и) ран(и|я)т себя на {value} HP.',
	...
	'[hit]{actor} оплета(е|ю)т корнями {victim} и пыта(е|ю)тся покалечить жертву. {} получил(а|о|и) {value} единиц(у|ы) урона.',
	...
	'[poison]Охваченн(ый|ая|ое|ые) ядовитыми парами, {victim} теря(е|ю)т {value} HP.',
	...
	'[poison]{victim} задыха(е|ю)тся и теря(е|ю)т {value} единиц(у|а|ы) здоровья.',
	'[poison]Раны {victim} пульсируют и источают смрад. Нанесены {value} единиц(у|а|ы) урона.',
	...
	'[eva]{victim} исполня(е|ю)т серию эффектных ударов, но ни один из них не достигает цели. {actor} сдержано аплодиру(е|ю)т.',
...
//	'[fire]{victim} не успел(а|о|и) увернуться от возникшего прямо из воздуха всполоха пламени. От ожогов (он|она|оно|они) потерял(а|о|и) {value} HP.', //err
	...
	'[fire]{actor} извл(ёк|екла|екло|екли) из подсумка запечатанную колбу и швырнул(а|о|и) её в противника. {victim} визж(и|а)т и пыта(е|ю)тся стряхнуть с себя алхимическое пламя, теряя {value} HP.',
	...
	'[stunHit]Стремителен бросок {actor} к противнику! {victim} на земле и лишен(а|о|ы) {value} единиц(у|ы) здоровья.',
	...
	'[rest]Медитация помогла, {} восстановил(а|о|и) {value} единиц(у|ы) здоровья.',
	'[rest]{} проходящего мимо шамана: «Ух ты! А бубном и вправду можно вылечить {value} единиц(у|ы) здоровья».',
	'[rest]Холодный компресс восстанавливает {value} единиц(у|ы) здоровья.',
...
//		'[coins]{} споткнул(ся|ась|ось|ись) о камень, а это оказался кошель с {value} монетами! Вот так повезло!',
	...
	'[pickup]{} повертела в мозолистых руках {item}, что-то прикидывая, затем тихо выругал(ся|ась|ось|ись) и убрал(а|о|и) добычу в рюкзак.',
...
//			'[drop]Короткая вспышка — и {} с удивлением смотр(ит|ят) на горстку пепла, оставшуюся от {drop}.', //manual drop
	...
	'[drop]«Эх, полным полна моя коробочка, - вздохнул(а|о|и) {}. - {} под этим камнем. Не забыть бы {} на обратном пути».',
	...
	'[death]{} наносит сокрушительный удар, и {} падает навзничь.',
	...
	'[fight]«Ну наконец-то, {}!" — воскликнул(а|о|и) {}. — А то у же тело зудит, как накостылять кому-то хочется!»',
