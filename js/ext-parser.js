...
//		'[coins]{} споткнул(ся|ась|ось|ись) о камень, а это оказался кошель с {value} монетами! Вот так повезло!',
		...
		'[coins]Заметив в моем инвентаре {item}, торговец в ужасе отпрянул и заплатил мне {value} монет(а|у|ы) за то, чтобы я выкинул эту мерзость подальше.',
		'[coins]Творчески использовав классику торговли — обсчет, обвес и пересорт, продал торговцу {item} за {value} монет.',
		...
		'[pickup]{} подбросил(а|о) {item} над головой. Т(от|о|а|е) перелетел(а|о|и) за спину и упал(а|о|и) прямо в рюкзак к остальной добыче. «В яблочко!» – довольно улыбнулся {}.',
		'[pickup]«Что-то упало. Я точно видел, как что-то упало с {}. Да вот только непонятно, куда? О, наш(ел|ла|ло)! {item}!»',
		'[pickup]Немного постояв над найденн(ым|ой|ыми) {item}, {} аккуратно его поднял(а|о) и спрятал(а|о) к себе в рюкзак.',
...
//			'[drop]Короткая вспышка — и {} с удивлением смотр(ит|ят) на горстку пепла, оставшуюся от {drop}.', //manual drop
		...
		'[fight]{} бросил камень в кусты, чем разозлил {}, стоявш(его|ую|ее|их) за ними.', 
