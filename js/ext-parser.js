var _parse = (function(_parse) {
	...
	var parseShortRaw = [
		...
		'[hit]Вложив всю злость в удар, {actor} нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		'[hit]{actor} точным ударом поразил(а|о|и) {victim} на {value} единиц(у|ы) урона.',
		'[hit]{actor} пров(е|ё)л(а|о|и) сногсшибательный при(е|ё)м, {victim} пада(ет|ют) и теря(ет|ют) {value} HP.',
		'[hit]Усыпив бдительность {victim}, {actor} набросил(ся|ась|ось|ись) на противника и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		'[hit]После обмена серией стремительных ударов с {victim}, {actor} наконец наш(ел|ёл|ла|ло|ли) брешь в защите и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		'[hit]Рассекающий удар {actor} нан(е|ё)с {value} единиц(у|ы) урона {victim}.',
		'[hit]Кувырком уходя из-под удара и контратакуя, {actor} сумел(а|о|и) нанести {victim} {value} единиц(у|ы) урона.',
		'[hit]«Скрытая атака!» — заорал(а|о|и) {actor} и выбил(а|о|и) из {victim} {value} HP.',
		'[hit]Ловко парировав удар, {actor} пров(е|ё)л(а|о|и) успешный контрудар, лишая {victim} {value} HP.',
		...
		'[hit]Замахиваясь, {victim} пропустил(а|о|и) резкий и быстрый выпад {actor}, забравший {value} HP.',
		'[hit]{victim} приготовил(ся|ась|ось|ись) парировать удар, но не ожидал(а|о|и) от {actor} такой стремительности и теря(ет|ют) {value} HP.',
		'[hit]Стоило только {victim} замахнуться, как {actor} резко ударил(а|о|и) {victim} по больному месту и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		'[hit]{victim} пропуска(ет|ют) л(е|ё)гкий тычок в {value} единиц(у|ы) урона.',
		'[hit]Сосредоточившись, {actor} пров(е|ё)л(а|о|и) атаку и лишил(а|о|и) {victim} {value} HP.',
		'[hit]Быстрая атака {actor} лишила {victim} {value} HP.',
		'[hit]Обманув {victim}, {actor} смог(ла|ло|ли) провести неожиданную атаку, лишив {victim} {value} HP.',
		'[hit]{victim} потерял(а|о|и) {value} HP, пропустив атаку {actor}.',
		'[hit]{actor} нан(е|ё)с(ла|ло|ли) точный удар, уменьшая здоровье {victim} на {value} HP.',
		'[hit]{actor} виртуозно пров(е|ё)л(а|о|и) атаку и нан(е|ё)с(ла|ло|ли) {victim} {value} единиц(у|ы) урона.',
		...
		'[hit]Изучив тактику {victim}, {actor} пров(е|ё)л(а|о|и) атаку и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		'[hit]С кошачьей грацией {actor} извернул(ся|ась|ось|ись) и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		...
		'[hit]{actor} поцарапал(а|о|и) {victim} на {value} HP.',
		'[hit]Апперкот, и вокруг {victim} взметнул(ась|ось|ись) {value} зв(е|ё)здоч(ка|ки|ек).',
		'[hit]Умелая подсечка, и придорожный камень нан(е|ё)с голов(е|ам) {victim} {value} урона.',
		'[hit]{actor} поднатужил(ся|ась|ось|ись) и нан(е|ё)с(ла|ло|ли) {value} единиц(у|ы) урона.',
		...
		'[rest]«Ну и гадость!» — {actor} выпивает очередную порцию запас(е|ё)нного когда-то лечебного отвара.',
